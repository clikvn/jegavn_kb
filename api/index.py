...
# import google.generativeai as genai  # Removed - using Flowise only
import re
...
# Google GenAI removed - using Flowise only
...
async def flowise_chat(request: Request):
    ...
    try:
        ...
        try:
            ...
            question = body.get("question", "").encode('latin1').decode('utf-8')
            ...
            async with session.post(flowise_url, json=payload) as response:
                ...
                try:
                    ...
                    if "text" in data:
                        ...
                        logger.info(f"🔍 [FLOWISE] Session metadata - chatId: {metadata['chatId'][:8] if metadata['chatId'] else 'N/A'}..., chatMessageId: {metadata['chatMessageId'][:8] if metadata['chatMessageId'] else 'N/A'}...")
...
async def flowise_stream(request: Request):
...
# Chat function removed - using Flowise only
